...
final class BooksCallNumberController
{
    ...
    public function update(Request $request, $idLibro)
    {
        ...
        if ($xCollocazioneNuova !== 'null') {
            ...
            if ($libroTarget) {
                return to_route('books.call-number.swap', ['id' => $libro->id, 'idTarget' => $libroTarget->id]);
            }
            $libro->collocazione = $xCollocazioneNuova;
            $res = $libro->save();

            return to_route('books.show', ['id' => $libro->id])->withSuccess("La collocazione $collocazione è stata sostituita in $libro->collocazione con successo.");

        }
        $libro->collocazione = null;
        $libro->save();

        return to_route('books.show', ['id' => $libro->id])->withSuccess("La collocazione $collocazione è stata sostituita con SENZA COLLOCAZIONE con successo.");
    ...
    public function swapUpdate($idLibro, $idTarget)
    {
        ...
        return to_route('books.show', ['id' => $libro->id])
