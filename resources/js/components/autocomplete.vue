...
export default {
  ...
  props: {
    ...
    selected: {
      //object containing the selected items of the form ({id:name}) (e.g. {"275":"A. A. TARKOVSKIJ","374":"A. J. CRONIN","399":"A. MANZINO"}
      type: Object,
      default: () => ({}),
    },
    multiple: {
      //if "true" allow to select multiple choiches
      type: Boolean,
      default: false,
    },
    placeholder: {
  ...
  methods: {
    ...
    getOptions(search, loading) {
      loading(true);
      axios
        .get(this.url, { params: { term: search } })
        .then((response) => {
    ...
    changed: function (selectedValues) {
      ...
      if (this.multiple)
        // if mulitple=true, selectedValues is an array
        this.sentvalue = _.map(selectedValues, "value"); // from ["value":1, "value":2] to [1,2,3,4]
      // multiple=false than selectedValues is an object
      // this.sentvalue = selectedValues.value;
      else console.log(selectedValues);
