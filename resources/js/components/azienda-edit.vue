    ...
    <div class="row">
      <div class="col-md-8 table-responsive">
        <table class="table table-hover table-bordered">
          ...
          <tbody>
            <tr
              v-for="lavoratore in lavoratori"
              v-bind:id="lavoratore.id"
              hoverable
            >
              <td scope="row">
                {{ lavoratore.nominativo
                }}<span
                  v-bind:class="badgeMansione(lavoratore.pivot.mansione)"
                  >{{ lavoratore.pivot.mansione }}</span
                >
              </td>
              <td class="text-center">
                <span v-bind:class="badgeStato(lavoratore.pivot.stato)">{{
          ...
          <div class="card-body">
            <div class="list-group list-group-flush">
              <a
                ...
                v-for="storico in lavoratori_storici"
                v-bind:id="storico.id"
                >{{ storico.nominativo }}
                <span class="badge badge-danger badge-pill ml-4">{{
                  storico.pivot.data_fine_azienda
                }}</span
        ...
        <div class="modal-mask" @click="close" v-show="showModal">
          <div class="modal-container" @click.stop>
            ...
            <div class="modal-body">
              <div class="row">
                <label for="data_inizio" class="col-sm-4"
                  >Data inizio lavoro</label
                >
                <div class="col-sm-8">
                  <date-picker
                    id="data_inizio"
                    @selected="dateInizioSelected"
                    :value="data_inizio_lavoro"
                    :language="language"
              ...
              <div class="row" v-show="nonAttivoChecked">
                ...
                <div class="col-sm-8">
                  <date-picker
                    ...
                    id="data_fine"
                    @selected="dateFineSelected"
                    :value="data_fine_lavoro"
                    :language="language"
                    :format="customFormatter"
            ...
            <div class="modal-footer text-right">
              <button
                ...
                role="button"
                @click="salvaModificheLavoratore"
                v-bind:disabled="validateData"
              >
                salva
              </button>
          ...
          <div class="modal-container" @click.stop>
            ...
            <div class="modal-body">
              <div class="row">
                <div class="col-sm-8">
                  ...
                  <hr />
                  <div class="row">
                    <label for="data_fine_sposta" class="col-sm-4"
                      >Data fine lavoro:</label
                    >
                    <div class="col-sm-8">
                      <date-picker
                        placeholder="Selezionare una data"
                        ...
                        :language="language"
                        :format="customFormatter"
                        @selected="dateSpostaSelected"
                        :value="data_fine_sposta"
                        :disabledDates="dateDisabledSposta"
                      ></date-picker>
                    </div>
                  </div>
                  <hr />
                  <div class="row">
                    <label for="nuova_azienda" class="col-sm-4"
                      >Sposta in:</label
                    >
                    <div class="col-sm-8">
                      <select
                        ...
                        :value="nuova_azienda_id"
                        v-model="nuova_azienda_id"
                        @change="changeAziendaSposta"
                      >
                        <option value="" hidden>Seleziona un'azienda</option>
                        <option
                          v-for="azienda in aziendePossibili"
                          :value="azienda.id"
                        >
                          {{ azienda.nome }}
                        </option>
                ...
                <div class="col-sm-4">
                  <div class="card">
                    ...
                    <div class="card-body">
                      <p v-if="aziendeStorico.length == 0" class="text-danger">
                        Nussuna azienda nello storico
                      </p>
                      <ul v-else>
                        <li v-for="azienda in aziendeStorico">
                          {{ azienda.nome }}
                        </li>
                      </ul>
                    </div>
            ...
            <div class="row" v-show="!validateSposta">
              <div class="col-sm offset-sm-1">
                <p class="text-danger">
                  Data inizio lavoro in <strong>{{ nome_nuova_azienda }}</strong
                  >: <strong>{{ data_fine_sposta }}</strong>
                </p>
              </div>
            </div>
            <div class="modal-footer text-right">
              <button
                class="btn btn-success"
                role="button"
                :disabled="validateSposta"
                @click="salvaSpostaLavoratore"
              >
                salva
              </button>
              <button
      ...
      <transition name="modal">
        ...
        >
          <div class="modal-container" @click.stop>
            ...
            <div class="modal-body">
              <div class="row">
                <label for="nuovo_lavoratore" class="col-sm-4"
                  >Lavoratore:</label
                >
                <div class="col-sm-8">
                  <v-select
                  ...
                  >
                    <span class="text-danger" slot="no-options">{{
                      noPersone
                    }}</span>
                  </v-select>
                </div>
              ...
              <hr />
              <div class="row">
                <label for="inizio_lavoratore" class="col-sm-4"
                  >Data inizio lavoro:</label
                >
                <div class="col-sm-4">
                  <date-picker
            ...
            <div class="modal-footer text-right">
              <button
                class="btn btn-success"
                :disabled="validateAggiungi"
                @click="salvaNuovoLavoratore"
              >
                salva
              </button>
...
export default {
  ...
  created: async function () {
    await axios
      .get(this.url_azienda_edit)
      .then(
        (response) => (
          (this.nome = response.data[0].nome),
          (this.lavoratori = response.data[0].lavoratori),
          (this.tipo = response.data[0].tipo),
          (this.lavoratori_storici = response.data[0].lavoratoriStorici)
        ),
      );
    await axios
      .get(this.url_mansioni)
      .then((response) => (this.mansioni = response.data));
    await axios
      .get(this.url_stati)
      .then((response) => (this.stati = response.data));
  },
  computed: {
    noPersone: function () {
  ...
  methods: {
    changeAziendaSposta: function () {
      ...
      var r = this.aziendePossibili.find(
        (el) => el.id == this.nuova_azienda_id,
      );
    ...
    modificaLavoratore: function (id) {
      this.showModal = true;
      this.titoloModal =
        "Modifica lavoratore: " + this.lavoratori[id].nominativo;
    ...
    spostaLavoratore: async function (id) {
      const response = await axios.get(
        "/api/nomadelfia/aziende/lavoratore/" +
          this.lavoratori[id].id +
          "?filtro=storico",
      );
      ...
      await axios
        ...
        .then((response) => (this.aziendePossibili = response.data));
    ...
    salvaModificheLavoratore: function () {
      axios
        .post(this.url_modifica_lavoratore, {
      ...
      axios
        .get(
          this.url_search +
            ...
            search +
            "&azienda_id=" +
            Number(this.id_azienda),
        )
        .then((response) => (this.persone = response.data), loading(false));
    },
    salvaNuovoLavoratore: function () {
      axios
        .post(this.url_aggiungi, {
          azienda_id: this.id_azienda,
