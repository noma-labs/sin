      ...
      <div class="col-md-3">
        <div class="form-group">
          ...
          <input
            ...
            data-toggle="tooltip"
            title="nominativo utilizzato internamente per la persona. es: Matteo L."
          />
        </div>
      </div>
    ...
    <div class="form-row">
      <div class="form-group col-md-3">
        ...
        <select
          ...
          id="nucleo"
          name="nucleo"
          v-model="posizioni_selected"
        >
          <option selected hidden value="">Seleziona...</option>
          <option v-for="posizione in posizioni" v-bind:value="posizione.id">
            {{ posizione.posizione }}
          </option>
        </select>
      </div>
      <div class="form-group col-md-3" v-show="needFamiglia">
        ...
        <select id="famiglia" class="form-control" name="famiglia">
          ...
          <option v-for="famiglia in famiglie" v-bind:value="famiglia.id">
            {{ famiglia.nome }}
          </option>
        </select>
      </div>
...
export default {
  ...
  mounted: function () {
    axios
      .get("/api/nomadelfia/posizioni")
      .then((response) => (this.posizioni = response.data));
  },
  computed: {
    nomeIntero: function () {
  ...
  methods: {
    nuovaFamiglia: function () {
      axios
        .post("/api/nomadelfia/famiglie/create", {
