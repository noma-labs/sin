...
<div class="row">
    ...
    <div v-if="msgDisplay" class="alert alert-warning" role="alert">
        {{msg}}
        <button type="button" class="close" aria-label="Close" @click="closeMsg()">
        ...
        <tbody>
            <tr v-for="(f, i) in filtri" :key="f.id"> 
                ...
                <td>
                    <div class="row">
                        <div class="col">
                            <button class="btn btn-warning btn-sm btn-block" @click="showModal=true">Modifica</button>                            
                            <!-- <my-modal modal-title="Modifica Filtro" button-style="btn-block btn-warning btn-sm" button-title="Modifica"></my-modal> -->
                        </div>
                        <div class="col">
                            <button type="button" class="btn btn-danger btn-sm btn-block" @click="elimina(f.id, i)">Elimina</button>
                        </div>
                    </div>
                </td>
            </tr>
...
export default {
    ...
    mounted(){
        axios.get(this.ulrFiltri)
        .then((response) => {
            this.filtri = response.data;
        });
        axios.get(this.ulrTipi)
        .then((response) => {
    ...
    methods: {
        ...
        elimina: function(id, index) {
            axios.delete(this.urlElimina, {data: {filtro: id}}).then((response) => {
