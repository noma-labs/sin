    ...
    <div class="card card-mod">
        ...
        <div class="card-body card-body-mod">
            <ul class="list-group list-group-flush">
                <li v-for="(g, i) in gomme_local" v-bind:key="g.id" class="list-group-item">
                    <div class="row">
                        <div class="col-sm-10">
                            {{g.codice}} 
                        </div>
                        <div class="col-sm-2">
                            <button type="button" @click="eliminaGomma(i, g.id)" class="btn btn-danger btn-sm">
            ...
            <br>
            <my-modal  modal-title="Aggiungi Tipo Gomma" button-title="Aggiungi Gomma" button-style="btn-block btn-warning">
                <template v-slot:modal-body-slot>
                    <button class="btn btn-warning" type="button" @click="toggleMode" v-bind:disabled="esistente">Esistente</button>
                    <button class="btn btn-warning" type="button" @click="toggleMode" v-bind:disabled="nuova">Nuova</button>
                    <form method="POST" id="form-aggiungi-gomma">
                        <div class="row">
                            <div class="col-md-6">
                                <label for="codice">Codice Gomma</label>
                                <input v-if="nuova" name="codice" type="text" id="codice" v-model="codice_nuova_gomma" class="form-control" placeholder="es. 195/70 r15">
                                <select v-if="esistente" class="form-control" name="codice" type="text" v-model.lazy="codice_esistente_gomma">
                                    <option value="" disabled hidden>--Seleziona--</option>                                        
                                    <option v-for="g in lista_gomme" v-bind:key="g.id" v-bind:value="g.id">{{g.codice}}</option>                                        
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label for="note">Note</label>
                                <input type="text" name="note" id="note" class="form-control" v-model="note" placeholder="Nota Facoltativa...">
                            </div>
                        </div>
                    </form>
                    <br>
                ...
                <template v-slot:modal-button>
                    <button type="button" class="btn btn-success" @click="nuovaGomma">Salva</button>
                </template>
            </my-modal>
        </div>
    </div>
