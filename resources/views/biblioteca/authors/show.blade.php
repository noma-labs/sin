...
@section("content")
  ...
  <div class="row">
    <div class="col-md-6">
      ...
      <a
        href="{{ route("authors.edit", $autore->id) }}"
    ...
    <div class="col-md-6">
      <div class="card">
        <div class="card-header">Libri Scritti ({{ $books->count() }})</div>
        <div class="card-body">
          <ul>
            @foreach ($books->get() as $libro)
              <li>
                <a href="{{ route("books.show", ["id" => $libro->id]) }}">
                  {{ $libro->collocazione }} - {{ $libro->titolo }}
                </a>
              </li>
            @endforeach
          </ul>
