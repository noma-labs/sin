    ...
    <div class="row">
      <div class="col-md-3">
        <div class="">
          ...
          <select class="form-select" name="xSegnoInizioPrestito" type="text">
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="float-end m-1">
          ...
          <button class="btn btn-success" name="biblioteca" type="submit">
            Cerca Prestiti
          </button>
        </div>
      </div>
            ...
            <td>
              ...
              @if ($prestito->in_prestito)
                <span class="badge bg-warning">
                  {{ Carbon\Carbon::parse($prestito->data_inizio_prestito)->diffInDays(Carbon\Carbon::now(), false) }} giorni
                </span>
              @else
                <span class="badge bg-success">Restituito</span>
              @endif
            </td>
            <td>{{ $prestito->cliente->nominativo }}</td>
            <td>{{ $prestito->bibliotecario->nominativo }}</td>
            <td>{{ $prestito->libro->titolo }}</td>
            <td>{{ $prestito->libro->collocazione }}</td>
            <td>{{ $prestito->note }}</td>
            <td>
              <a
