      ...
      <div class="card-body">
          @if ($posattuale != null)
            ...
            <div class="row">
              ...
              <p class="col-md-2">{{$posattuale->pivot->data_inizio }} </p>
              <div class="col-md-2">
                <span class="badge badge-info"> @diffHumans($posattuale->pivot->data_inizio) </span>
               </div>
              <div class="col-md-5">
               @include("nomadelfia.templates.modificaDataPosizione",["persona"=>$persona, 'id'=>$posattuale->id, 'nome'=>$posattuale->nome, "data_inizio"=>$posattuale->pivot->data_inizio])
                ...
                <my-modal modal-title="Concludi Posizione" button-title="Concludi" button-style="btn-info my-2">
                    <template slot="modal-body-slot">
                        <form class="form" method="POST"  id="formConcludiPosizione{{$posattuale->id}}" action="{{ route('nomadelfia.persone.posizione.concludi', ['idPersona' =>$persona->id, 'id'=>$posattuale->id]) }}" >      
                            ...
                            <div class="form-group row">
                              ...
                              <div class="col-sm-6">
                                <date-picker :bootstrap-styling="true" value="{{$posattuale->pivot->data_fine }}" format="yyyy-MM-dd" name="data_fine"></date-picker>
                              </div>
                            </div>
                    ...
                    <template slot="modal-button">
                      <button class="btn btn-success" form="formConcludiPosizione{{$posattuale->id}}" >Salva</button>
                    </template>
                </my-modal> 
          ...
          <my-modal modal-title="Aggiungi Posizione persona" button-title="Nuova Posizione" button-style="btn-success  my-2">
              <template slot="modal-body-slot">
                <form class="form" method="POST"  id="formPersonaPosizione" action="{{ route('nomadelfia.persone.posizione.assegna', ['idPersona' =>$persona->id]) }}" >      
                    {{ csrf_field() }}
                    @if($posattuale != null)
                    <h5 class="my-2">Completa dati della posizione attuale: {{$posattuale->nome}}</h5>
                    <div class="form-group row">
