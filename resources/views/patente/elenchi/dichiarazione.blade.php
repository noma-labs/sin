...
<h3 class="text-right py-5">Nomadelfia, {{Carbon::now()->format('d/m/Y')}}
    <h3>

        <h3 class="text-left py-5">
            Il sottoscritto
            @if($presidente->cognome)
            {{$presidente->cognome}} {{$presidente->nome}} nato a {{$presidente->provincia_nascita}} il
            {{$presidente->data_nascita}},
            @else
            ____________ nato a _______ il _________,
            @endif
            residente in Via Nomadelfia 1, Roselle 58100 Grosseto (GR), in qualità di Presidente pro-tempore della
            Associazione Nomadelfia, con sede in Grosseto
        </h3>
