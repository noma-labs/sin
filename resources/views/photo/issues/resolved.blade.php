        ...
        <div class="card border-secondary">
            <div class="card-header bg-secondary text-white">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        Problema risolto {{ $issues->currentPage() }} di
                        {{ $issues->lastPage() }}
                    </div>
                </div>
            </div>
                            ...
                            <dl class="row mb-3">
                                ...
                                <dd class="col-sm-7">
                                    <span class="badge text-bg-success fs-6">
                                        {{ \Illuminate\Support\Carbon::parse($issue->resolved_at)->format("d/m/Y") }}
                                    </span>
                                </dd>
                                @if (count($issue->date_changes) > 0)
                                    <dt class="col-sm-5">Modifica data</dt>
                                    <dd class="col-sm-7">
                                        @foreach ($issue->date_changes as $change)
                                ...
                                @if (count($issue->plain_notes) > 0)
                                    ...
                                    <dd class="col-sm-7">
                                        @foreach ($issue->plain_notes as $noteLine)
                                            <small class="text-muted d-block">{{ $noteLine }}</small>
                                        @endforeach
                                    </dd>
                                @endif
