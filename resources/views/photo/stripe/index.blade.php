...
@section("content")
  ...
  <div class="d-flex flex-wrap gap-2 my-3">
    ...
    @forelse ($years as $year)
      ...
      >
        {{ $year->year }}
        <span class="badge text-bg-secondary"> {{ $year->count }} </span>
      </a>
    @empty
      <p>Nessun anno</p>
    @endforelse
              ...
              <div class="d-flex align-items-center w-100 gap-3">
                ...
                @if (! is_null($stripe->nfo))
                  <span>
                    ...
                    <span class="fw-semibold"> {{ $stripe->nfo }} </span>
                  </span>
                  @if ($photoCount == 0)
                  ...
                  @elseif ($photoCount < $stripe->nfo)
                    <span class="badge text-bg-warning">
                      {{ $stripe->nfo - $photoCount }} foto mancanti
                    </span>
                  @elseif ($photoCount > $stripe->nfo)
                    <span class="badge text-bg-danger">
                      {{ $photoCount - $stripe->nfo }} foto in più
                    </span>
                  @endif
                @endif
              ...
              <div class="mb-3 d-flex flex-wrap gap-3 align-items-center">
                @if (! empty($stripe->localita))
                  <span>
                    ...
                    <span class="fw-semibold"> {{ $stripe->localita }} </span>
                  </span>
                @endif
                ...
                @if (! empty($stripe->argomento))
                  <span>
                    ...
                    <span class="fw-semibold"> {{ $stripe->argomento }} </span>
                  </span>
                @endif
                    ...
                    >
                      <figure class="figure m-1" style="width: 18rem">
                        <div class="position-relative">
                          ...
                          >
                            <div class="small">
                              {{ $photo->file_name }} {{ $photo->taken_at ? $photo->taken_at->format("Y-m-d") : "N/A" }}
                            </div>
