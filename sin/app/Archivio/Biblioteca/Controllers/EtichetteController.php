...
class EtichetteController extends CoreBaseController
{
  ...
  public function printToPdf(){
    ...
    $data = Carbon::now();
    return $pdf->setPaper('a4')->download("etichette-$data.pdf"); 
  }
  ...
  public function removeLibro($idLibro){
    ...
    if($res) return redirect()->route('libri.etichette')->withSuccess("Libro $libro->collocazione, $libro->titolo eliminato dalla stampa delle etichette");
    else  return redirect()->route('libri.etichette')->withError("Errore nell'operazione");
  }
  ...
  public function removeAll(Request $request){
      ...
      if($res) return redirect()->route('libri.etichette')->withSuccess("Tutte le $res etichette sono state eliminate.");
      else  return redirect()->route('libri.etichette')->withError("Errore nell'operazione");
   }
