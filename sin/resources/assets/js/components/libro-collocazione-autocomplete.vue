         ...
         <div class="form-group">
            ...
            <select class="form-control" v-model="selectedNumeri" >
              ...
              <optgroup v-if="numeri.numeriAssegnati != null" label="Numeri assegnati" >
                <option v-for="numero in numeri.numeriAssegnati" v-bind:value="numero">
                  {{ numero }}
                </option>
              </optgroup>
              <optgroup v-if="numeri.numeriMancanti != null" label="Numeri liberi">
                <option v-for="numero in numeri.numeriMancanti" v-bind:value="numero">
                  {{ numero }}
                </option>
              </optgroup>
...
<script>
  ...
  export default {
    ...
    props: {
     ...
     numeriAssegnati: {  //mostra solo i numeri asseganti oppure tutti i numeri
       type: String,
       default: "true",
     },
     numeriMancanti: {  //mostra solo i numeri asseganti oppure tutti i numeri
       type: String,
       default: "true",
     },
     numeroNuovo: {  //mostra solo i numeri asseganti oppure tutti i numeri
       type: String,
       default: "true",
   },
   },
    ...
    computed : {
      ...
      selectedCollocazione : function(){
        ...
        if(this.numeriRequired == "true" && !this.selectedNumeri) // return the collocazione only if numeri has been selected
          return null;
        else
          return this.selectedLettere+this.pad(this.selectedNumeri,3); // generate the XXXNNN collocazione hwere X is letter and N a number
    ...
    methods: {
      getOptions(search, loading) {
        loading(true)
        axios.get(this.urlLettere, { params: { term: search } })
              .then(response => {
      ...
      changed: function(value){
        ...
        axios.get(this.urlLettere, { params: { lettere: value.label, nuovo: this.numeroNuovo, mancanti: this.numeriMancanti, assegnati: this.numeriAssegnati} })
              .then(response => {
      ...
      pad: function(a,b) //a // the number to convert // number of resulting characters
          {
            if(a)
            return (
