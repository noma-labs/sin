    ...
    <div class="col-md-8">
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <input type="text" class="form-control" id="autore" value="{{$libro->classificazione->descrizione}}"
                       disabled>
                @endif
            </div>
        </div>
        ...
        <!-- End note  class-->
        <form id="addLibroPrint" action="{{route('libri.etichette.aggiungi.libro',['idLibro' => $libro->id])}}"
              method="post">
            {{ csrf_field() }}
        </form>

        <form id="removeLibroPrint" action="{{route('libri.etichette.rimuovi.libro',['idLibro' => $libro->id])}}"
              method="post">
            {{ csrf_field() }}
        </form>
    ...
    <div class="col-md-4">
        ...
        <div class="card card-info border-info ">
            ...
            <div class="card-body">
                ...
                @if ($libro->inPrestito())
                <a class="btn btn-primary" href="{{route('libri.prestito', ['idPrestito' => $prestito->id])}}">Gestisci
                    prestito</a>
                @else
                <a class="btn btn-primary" href="{{route('libri.prenota', ['idLibro' => $libro->id])}}">Dai in
                    Prestito</a>
                @endif
            </div>
        </div>
                    ...
                    <div class="media">--}}
                        ...
                        <div class="media-left">--}}
                            ...
                            {{-- <a href="{{ $file->getUrl() }}" target="_blank">--}}
                                {{-- <img class="media-object" style="width:75px" src="{{ $file->getUrl() }}"
                                          alt="{{ $file->name }}">--}}
                                {{-- </a>--}}
