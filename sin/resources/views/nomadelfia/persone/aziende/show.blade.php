   ...
   <div class="col-md-8">
      ...
      <div class="card-body">
          ...
          @forelse ($persona->aziendeAttuali as $azienda)
            <div class="row">
              ...
              <div class="col-md-2">
                <span class="badge badge-info"> @diffHumans($azienda->pivot->data_inizio_azienda)</span>
               </div>
               <div class="col-md-2">
                <my-modal modal-title="Modifica Azienda attuale" button-title="Modifica" button-style="btn-warning my-2">
                  <template slot="modal-body-slot">
                    <form class="form" method="POST"  id="formPersonaGruppoModifica" action="{{ route('nomadelfia.persone.aziende.modifica', ['idPersona' =>$persona->id, 'id'=>$azienda->id]) }}" >      
                        {{ csrf_field() }}
                        ...
                        <div class="form-group row">
                          ...
                          <div class="col-sm-6">
                            <date-picker :bootstrap-styling="true" value="{{$azienda->pivot->data_inizio_azienda }}" format="yyyy-MM-dd" name="data_entrata"></date-picker>
                          </div>
                        </div>
                        <div class="form-group row">
                          <label for="inputPassword" class="col-sm-6 col-form-label">Data fine</label>
                          <div class="col-sm-6">
                            <date-picker :bootstrap-styling="true" value="{{$azienda->pivot->data_fine_azienda }}" format="yyyy-MM-dd" name="data_uscita"></date-picker>
                          </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-6">
                              <select name="mansione" class="form-control">
                                  ...
                                  <option value="LAVORATORE" {{ $azienda->pivot->mansione == "LAVORATORE" ? 'selected' : '' }}> Lavoratore</option>
                                  <option value="RESPONSABILE AZIENDA" {{ $azienda->pivot->mansione == "RESPONSABILE AZIENDA" ? 'selected' : '' }}> Responsabile azienda</option>
                              </select>
                            </div>
                          </div>
                        <div class="form-group row">
                          ...
                          <div class="col-sm-6">
                            <div class="form-check">
                              <input class="form-check-input" type="radio" name="stato" id="forstatoM" value="Attivo" @if($azienda->pivot->stato=='Attivo') checked @endif>
                                <label class="form-check-label" for="forstatoM">
                                  Attivo
                                </label>
                            </div>
                            <div class="form-check">
                              <input class="form-check-input" type="radio" name="stato" id="forstatoM" value="Sospeso" @if($azienda->pivot->stato=='Sospeso') checked @endif>
                                <label class="form-check-label" for="forstatoM">
                                  Attivo
                                </label>
                            </div>
                            <div class="form-check">
                              <input class="form-check-input" type="radio" name="stato" id="forstatoF"  value="Non Attivo" @if($azienda->pivot->stato=='Non Attivo') checked @endif>
          ...
          <my-modal modal-title="Aggiungi Azienda" button-title="Nuova Azienda" button-style="btn-success my-2">
            <template slot="modal-body-slot">
            <form class="form" method="POST"  id="formPersonaAzinedaAggiungi" action="{{ route('nomadelfia.persone.aziende.assegna', ['idPersona' =>$persona->id]) }}" >      
                ...
                <div class="form-group row">
                  <label class="col-sm-6 col-form-label">Data entrata azienda</label>
                  <div class="col-sm-6">
    ...
    <div class="card my-3">
      ...
      <div class="card-body">
        ...
        <div class="row">
          <p class="col-md-3"> {{$aziendaStorico->nome_azienda}}</p>
          <p class="col-md-3">{{$aziendaStorico->pivot->data_inizio_azienda }} </p>
          <p class="col-md-3">{{$aziendaStorico->pivot->data_fine_azienda }} </p>
          <div class="col-md-3">
            <span class="badge badge-info"> 
            {{Carbon::parse($aziendaStorico->pivot->data_fine_azienda)->diffForHumans(Carbon::parse($aziendaStorico->pivot->data_inizio_azienda),['short' => true])}}</span>
            </div>
        </div>
