<my-modal modal-title="Sposta in un nuovo Gruppo Familiare" button-title="Sposta Gruppo" button-style="btn-success my-2">
   <template slot="modal-body-slot">
    <form class="form" method="POST"  id="spostaPersonGruppo{{$persona->id}}" action="{{ route('nomadelfia.persone.gruppo.sposta', ['idPersona' =>$persona->id, 'id'=>$attuale->id]) }}" >      
        {{ csrf_field() }}

        <h5 class="my-2">Completa dati del gruppo attuale:  {{$attuale->nome}}</h5>
        <div class="form-group row">
            <label for="inputPassword" class="col-sm-6 col-form-label">Data uscita gruppo familiare</label>
            <div class="col-sm-6">
            <date-picker :bootstrap-styling="true" value="{{ old('current_data_uscita') }}" format="yyyy-MM-dd"name="current_data_uscita"></date-picker>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-6">
            <date-picker :bootstrap-styling="true" value="{{ old('new_data_entrata') }}" format="yyyy-MM-dd"name="new_data_entrata"></date-picker>
            </div>
        </div>
        </form>
    </template> 
    <template slot="modal-button">
        <button class="btn btn-success" form="spostaPersonGruppo{{$persona->id}}" >Salva</button>
    </template>
</my-modal> <!--end modal-->