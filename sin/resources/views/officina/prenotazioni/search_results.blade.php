...
<div class="alert alert-warning alert-dismissible fade show" role="alert" >Ricerca effettuata:<strong> {{$msgSearch}}</strong>
 ...
 <div id="results" class="alert alert-success"> Numero di prenotazioni trovate: <strong> {{ $prenotazioni->total() }} </strong></div>
      ...
      <tr hoverable>
        ...
        <td>
          <div class='button-group btn-block' role='group'>
            <a role='button' class="btn btn-warning" href="{{ route('officina.prenota.modifica', $pren->id) }}">Mod.</a>
            <a role='button' class="btn btn-danger" href="{{ route('officina.prenota.delete', $pren->id) }}">Eli.</a>
          </div>
        </td>
      </tr>
      @endempty
